...
impl TodoListTopics {
    ...
    fn add(&mut self, topic: String, description: String) {
        let todo = Todo {
            description,
            completed: false,
        };
        self.items[topic].insert(self.next_id[topic], todo);
        self[topic].next_id += 1;
        self.save_to_file();
    }
    ...
    loop {
        ...
        match input {
            ...
            "list" => {
                ...
                todo_list.display(current_topic);
            }
            "change" => {
